...
add_filter('content_save_pre', 'Auto_Save_Image_markdown');
...
// auto save image for markdown
function Auto_Save_Image_markdown($content) {
	//保存图片
	if($_POST['Auto_Save_Image']=="1"){
		...
		$img_inline = array();
		preg_match_all("/!\[(.+)\]\((http:\/\/(.+?))( .+|\))/is",$content1,$img_inline);
		$img_ref = array();
		preg_match_all("/\[(\d+)\]: (http:\/\/(.+?))( .+|$)/is",$content1,$img_ref);
		
		$img_inline = dhtmlspecialchars($img_inline[2]);
		$img_ref = dhtmlspecialchars($img_ref[2]);
		
		$img_array = array_combine($img_inline,$img_ref);
		$img_array = array_unique($img_array);
		$mapper = doFetchRemoteImage($img_array);
		foreach ($mapper as $key => $value){
			$content1 = str_replace($key,$value,$content1); //替换文章里面的图片地址
		}
...
/**
 * @param img_array unique dhtmlspecialchars array
 */
function doFetchRemoteImage($img_array) {
	...
	foreach ($img_array as $key => $value){
		...
		if(str_replace(get_bloginfo('url'),"",$value)==$value&&str_replace(get_bloginfo('home'),"",$value)==$value){
			...
			if (in_array($fileext, $savefiletype)){ 
				...
				$mapper[$value] = get_bloginfo('url').$filepath.$filename.".".$fileext; //图片地址Map
			}
		}
	}
	return $mapper;
