...
<div id="page">
    <div id="header">
        <h1>PHP探针</h1>
        <div id="download"><A HREF="http://down.yahei.net/code/tz/tz.zip">探针下载</A></div>
    </div>
...
<?php preg_match_all( "/([^\s]+):[\s]{0,}(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/", $strs[$i], $info );?>
     <tr>
        ...
        <td width="35%">已接收 : <font color='#CC0000'><span id="NetInput<?=$i?>">0</span></font></td>
        <td width="35%">已发送 : <font color='#CC0000'><span id="NetOut<?=$i?>">0</span></font></td>
    </tr>
<?php endfor; ?>
</table>
<?php endif; ?>
