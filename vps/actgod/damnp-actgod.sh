...
deb-src http://mirror.peer1.net/debian/ testing main #contrib non-free
deb http://security.debian.org/ testing/updates main
END
    apt-get -q -y update
		invoke-rc.d nginx stop
		apt-get -q -y remove nginx
		apt-get -q -y --force-yes install nginx-light
			if [ ! -d /etc/nginx ];
        then
        mkdir /etc/nginx
	fi
...
*)
    ...
    for option in system exim4 mysql nginx php wordpress typecho ssh addnginx stable testing dhost vhost httpproxy eaccelerator  apache addapache sshport phpmyadmin
    do
        echo '  -' $option
    done
    ;;
